...
Display how many times the encrypted code was the same as the access code and
the amount of times is wasn't. A user can't encrypt or decrypt their code twice.
A user can encrypt and decrypt the same code repetitively, however a user can
only get the same code correct or incorrect once.
...
int main(void) {
    ...
    do {
        ...
        // gets the users choice from with in the menu options, returns -1 if
        // not an option
        ...
        case 2: {
            ...
            if (code_created && !code_encrypted) {
                encrypt_and_verify_code(user_code, access_code, &success,
                                        &failure,
                                        &code_created_decyption_check);
                ...
                printf("\n\aERROR - Please enter an unencrypted code and don't "
                       "encrypt the same code twice\n");
            }
            // end if
        ...
        case 3: {
            ...
            if (code_encrypted) {
                ...
                printf("\n\aERROR - Please enter an encrypted code and don't "
                       "decrypt the same code twice\n");
            }
            // end if
...
void menu() {
    ...
    printf("\n\t4. Display the number of times the encrypted code entered "
           "matches the authorised code (i) Successfully (ii) Incorrectly");
    printf("\n\t5. Exit Program");
    printf("\n\n---------------------------------------------------------------"
           "----------------------------------------------------------------");
}
...
int input_and_validation(int *min, int *max) {
    ...
    // convert the input using strtol which returns the input in a base 10 long
    // and assigns the memory address of the string remaning to a given pointer
...
void encrypt_and_verify_code(int user_code[], const int access_code[],
...
int exit_program() {
    ...
    do {
        // if user doesnt a enter a correct input the first time display an
        // \aERROR
        if (error_check == 1) {
